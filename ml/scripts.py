    ...
    def start(self):
        ...
        while True:
            ...
            for filename in files:
                ...
                #print(res)
                tmp_id = int('100' + filename[0] * (filename[0].isdigit()) + '0' * (not filename[0].isdigit()))
                res_dict = dict()
                res_dict['containers'] = []
                for r in res:
                    tmp_dict = dict()
                ...
                #print(tmp_id)
                r = requests.post(f'http://3.17.12.94/api/cameras/{tmp_id}', json=res_dict, headers={'Content-Type': 'application/json'})
                print(r, r.text)              
            sleep(60)
